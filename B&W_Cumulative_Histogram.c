...
unsigned char chi;
int *io,*ir,LUT[256];
long int hist[256], mmax, histc[256];
int nbg;
...
histc[0]=hist[0];
for (k=1;k<dimx;k++) histc[k]=histc[k-1]+hist[k];


//Cumulative histogram creation
for(k=0;k<255;k++) LUT[k]=(int)((histc[k]*(long int)255)/histc[255]);


for(j=0;j<255;j++){
	for(i=0;i<(dimy-LUT[j]);i++) ir[i*dimx+j]=0;
	for(i=(dimy-LUT[j]);i<dimy;i++) ir[i*dimx+j]=255;
}
